...
const CreateInvoiceFromJobs = () => {
  ...
  const handleReviewInvoice = () => {
    const itemsToInvoice = Object.entries(selected)
      ...
      .map(([jobId, jobData]) => {
        ...
        return {
          ...
          title: job?.address || 'Service',
          address: job?.address || '',
          job_type: job?.job_type || 'Service Call',
          arrival_time: job?.arrival_time || new Date().toISOString(),
    ...
    // Navigate to review page with job data
    navigate('/agent/invoices/review', { state: { items: itemsToInvoice, total: totalAmount } });
  };
              ...
              <div key={job.id} className={`p-4 flex flex-col md:flex-row items-start md:items-center gap-4 ${selected[job.id] ? 'bg-v3-bg-dark' : ''}`}>
                <div className="flex items-center gap-4 flex-shrink-0 cursor-pointer" onClick={() => handleToggleJob(job)}>
                  ...
                  <div>
                    ...
                    {job.job_type && <p className="text-sm text-v3-text-muted">{job.job_type}</p>}
                  </div>
                </div>
